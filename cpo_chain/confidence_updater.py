...
class ConfidenceUpdater:
    ...
    def run(self, limit=50, dry_run=False) -> dict:
        """
            ...
            for i, row in enumerate(rows):
                try:
                    ...
                    if not dry_run and (edgar_changed or news_changed):
                        ...
                        # confidence = MIN(1.0, base_score + edgar_score + news_score)
                        # base_score 預設為 0.5
                        base_score = 0.5 # Default base
                        ...
                        if news_changed:
                            conn.execute("""
                                ...
                                (relation_id, source, boost_value, status, snippet)
                                VALUES (?, ?, ?, 'success', ?)
                            """, (row["id"], news_source, new_news - row["news_score"], f"Found news via {news_source}"))
                    ...
                    errors += 1
                    # Record error in audit if possible
                    if not dry_run:
                        conn.execute("""
