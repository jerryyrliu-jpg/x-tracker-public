...
def build_all_tickers_prompt(tweets: list, days: int) -> str:
    ...
    return (
        ...
        "請依下列格式輸出繁體中文總結。嚴格遵守格式，不要加入任何額外符號或分隔符。\n\n"
        "輸出格式範例：\n"
        "**$AAPL**\n"
        "情緒：Bullish\n"
        "主要觀點：蘋果供應鏈訂單強勁，看好後市。\n"
        ...
        "\n"
        "規則：\n"
        "1. 找出所有被提及的標的（股票、加密貨幣、ETF 等）\n"
        "2. 每個標的按上述格式輸出，標的間以一個空白行分隔，欄位間不加任何符號\n"
        "3. 若某推文列出多個標的（如 ETF 成分股、觀察名單），\n"
        "   以該 ETF／清單本身為主條目，被提及的成分股列在「成分股：」一行，\n"
        "   不要把成分股獨立列為主條目\n"
        "4. 若同一標的在不同天有不同立場，在主要觀點後加註（轉變：...）\n"
        "5. 不要引用或貼出原始推文內容\n"
    )
