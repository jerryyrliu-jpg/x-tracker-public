...
def mock_db(tmp_path):
    ...
    conn.execute("CREATE TABLE industry_relations (id INTEGER PRIMARY KEY, from_company_id INTEGER, to_company_id INTEGER, edgar_score REAL DEFAULT 0.0, news_score REAL DEFAULT 0.0, confidence REAL DEFAULT 0.5)")
    conn.execute("CREATE TABLE confidence_audit (id INTEGER PRIMARY KEY, relation_id INTEGER, source TEXT, boost_value REAL, status TEXT, snippet TEXT)")
